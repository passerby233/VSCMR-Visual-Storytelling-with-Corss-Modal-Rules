...
class VISTDataset(Dataset):
    def __init__(self, opt):
        ...
        print('DataLoader loading semantics json file: ', opt.semantics_json)
        self.semantics_line = json.load(open(opt.semantics_json))
    ...
    def __getitem__(self, index):
        if self.task == 'story_telling':
            ...
            semantics = self.semantics_line[self.mode]
            ...
            # load semantic
            semantic = np.zeros(self.opt.semantic_length, dtype='int64')
            semanlst = semantics[story_id]
            semantic[:min(len(semanlst), self.opt.semantic_length)] = np.array(semanlst) - 2048
