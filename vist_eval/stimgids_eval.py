...
class StimgidsEvaluator:
    ...
    def evaluate(self, preds, measure=None):
        ...
        for story in self.vist_sis.stories:
            story_img_ids = '_'.join([str(img_id)
                                      for img_id in story['img_ids']])
        ...
        for stimgids in stimgids_to_Res.keys():
            ...
            for story in related_stories:
                ...
                gd_story_str = ' '.join(
                    [self.vist_sis.Sents[sent_id]['text'] for sent_id in gd_sent_ids])
                gd_story_str = gd_story_str.encode(
                    'ascii', 'ignore').decode('ascii')  # ignore some weird token
        ...
        for scorer, method in scorers:
            ...
            score, scores = scorer.compute_score(
                self.stimgids_to_Gts, self.stimgids_to_Res)
            if isinstance(method, list):
                ...
                self.setStimgidsToEval(
                    scores, self.stimgids_to_Gts.keys(), method)
    ...
    def setEvalStimgids(self):
        self.eval_stimgids = [
            eval for stimgids,
            eval in self.stimgids_to_eval.items()]
...
if __name__ == '__main__':
    ...
    from pprint import pprint
    import sys
    sys.path.append("../vist_api")
    from vist import Story_in_Sequence

    sis = Story_in_Sequence(
        '/home/lijiacheng/vist/images/',
        '/home/lijiacheng/vist/annotations/')
    ...
    for story in sis.stories[:1]:
        ...
        pred_story_str = ' '.join([sis.Sents[sent_id]['text']
                                   for sent_id in story['sent_ids']])
