...
class Evaluator:
    def __init__(self, opt, mode='val'):
        ...
        self.output_file = os.path.join(self.save_dir, 'test_{}')
    ...
    def eval_story(self, model, crit, dataset, loader, opt, side_model=None):
        ...
        prediction_txt = open(self.prediction_file, 'wb')  # open the file to store the predictions
        ...
        for iter, batch in enumerate(loader):
            ...
            if side_model is not None:
                ...
                if conv_feature is not None:
                    ...
                    results, _ = model.predict(feature_fc, semantic, beam_size=opt.beam_size)
    ...
    def test_challange(self, model, dataset, loader, opt, side_model=None):
        ...
        predictions = {"team_name": "",
                       "evaluation_info": {"additional_description": ""},
                       "output_stories": []}
            ...
            for j, story in enumerate(stories):
                ...
                if concat_flickr_id not in finished_flickr_ids:
                    ...
                    predictions['output_stories'].append(
                        {'story_id': story_id, 'album_id': album_id,
                         'photo_sequence': flickr_id, 'story_text_normalized': story})
