...
class Story_in_Sequence:
    def __init__(self, images_dir, annotations_dir, splits=None):
            ...
            info = json.load(
                open(
                    osp.join(
                        ...
                        'sis',
                        split +
                        '.story-in-sequence.json')))
            print('sis\'s [%s] loaded. It took %.2f seconds.' %
                  (split, (datetime.now() - b).total_seconds()))
        ...
        for img in sis['images']:
            ...
            img_id = img['id']
            album_to_img_ids[album_id] = album_to_img_ids.get(
                album_id, []) + [img_id]
        ...
        for sent_id, sent in self.Sents.items():
            ...
            story_to_sent_ids[story_id] = story_to_sent_ids.get(
                story_id, []) + [sent_id]
        ...
        for story_id, sent_ids in story_to_sent_ids.items():
            ...
            album_to_story_ids[album_id] = album_to_story_ids.get(
                album_id, []) + [story_id]
    ...
    def show_story(self, story_id, show_image=True, show_sents=True):
        ...
        if show_sents:
            for sent_id in sent_ids:
                ...
                print(
                    '%s: img_id[%s], %s' %
                    (sent['order'], sent['img_id'], sent['text']))
...
class Description_in_Isolation:
    def __init__(self, images_dir, annotations_dir, splits=None):
            ...
            info = json.load(
                open(
                    osp.join(
                        ...
                        'dii',
                        split +
                        '.description-in-isolation.json')))
            print('dii\'s [%s] loaded. It took %.2f seconds.' %
                  (split, (datetime.now() - b).total_seconds()))
            for album in info['albums']:
                album['split'] = split
        ...
        for sent in sents:
            img_id = sent['img_id']
            img_to_sent_ids[img_id] = img_to_sent_ids.get(
                img_id, []) + [sent['id']]
    ...
    def show_imgs_with_sents(self, img_ids, show_image=True):
        ...
        for i, img_id in enumerate(img_ids):
            ...
            for k, sent_id in enumerate(sent_ids):
                ...
                print(
                    '%s.%s: img_id[%s], %s' %
                    (sent['order'], k, sent['img_id'], sent['text']))
