...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, weights=None, compute_prob=False):
        if len(target.size()) == 3:  # separate story
            input = input.view(-1, input.size(2), input.size(3))  # [batch_size*5, seq_length, vocab_size]
            target = target.view(-1, target.size(2))  # [batch_size*5, seq_length]

        seq_length = input.size(1)
        # truncate to the same size
        target = target[:, :input.size(1)]
        mask = (target > 0).float()  # [batch_size*5, seq_length]
        mask = to_contiguous(torch.cat([Variable(mask.data.new(mask.size(0), 1).fill_(1)), mask[:, :-1]], 1))

        # reshape the variables
        input = to_contiguous(input).view(-1, input.size(2))  # [batch_size*5*seq_length, vocab_size]
        target = to_contiguous(target).view(-1, 1)  # [batch_size*5*seq_length, 1]
        mask = mask.view(-1, 1)
