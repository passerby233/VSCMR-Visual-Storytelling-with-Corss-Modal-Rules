...
class BaseModel(nn.Module):
    def __init__(self, opt):
        ...
        self.seman_len = opt.semantic_length
        ...
        if self.rnn_type == 'gru':
            self.decoder = nn.GRU(input_size=self.word_embed_dim * 2, hidden_size=self.hidden_dim, batch_first=True)
        elif self.rnn_type == 'lstm':
            self.decoder = nn.LSTM(input_size=self.word_embed_dim * 2, hidden_size=self.hidden_dim, batch_first=True)
        else:
        ...
        # semantic attention
        self.bilinear = nn.Bilinear(self.word_embed_dim, self.hidden_dim, 1, bias=False)
        self.softmax = nn.Softmax(dim=1)
        self.seman_proj = nn.Sequential(nn.Linear(self.word_embed_dim, self.word_embed_dim),
                                        nn.Tanh())
    ...
    def semantic_attention(self, semantic_embed, state_d, semantic_mask):
        #  semantic_embed [batch_size*5, seman_len, embed_size]
        #  state_d [1, batch_size*5, hidden_size]
        semantic_embed_flatten = semantic_embed.view(-1, semantic_embed.size(2))
        state_d_expand = state_d.squeeze(0).unsqueeze(1).expand(-1, self.seman_len, -1).contiguous()
        state_d_expand = state_d_expand.view(-1, state_d_expand.size(2))
        weight = self.bilinear(semantic_embed_flatten, state_d_expand)
        weight = weight.view(-1, self.seman_len) + semantic_mask
        weight = self.softmax(weight).unsqueeze(2)
        context = torch.sum(semantic_embed * weight, dim=1)
        context = self.seman_proj(context).unsqueeze(1)
        return context

    def decode(self, imgs, last_word, state_d, context, penalize_previous=False):
        ...
        input_d = torch.cat([self.project_d(input_d), context], 2)
    ...
    def forward(self, features, caption, semantic):
        """
        :param features: (batch_size, 5, feat_size)
        :param caption: (batch_size, 5, seq_length)
        :param semantic: (batch_size, seman_len)
        :return:
        """
        # encode the visual features
        out_e, _ = self.encoder(features)  # [batch_size, 5 ,embed_size]

        # reshape the inputs, making the sentence generation separately
        out_e = out_e.view(-1, out_e.size(2))  # batch_size*5, emebed_size]
        caption = caption.view(-1, caption.size(2))  # [batch_size*5, seq_length=30]

        # setup semantic attention
        semantic = semantic.unsqueeze(1).expand(-1, 5, -1).contiguous()  # [batch_size, 5, seman_len]
        semantic = semantic.view(-1, semantic.size(2))  # [batch_size*5, seman_len]
        semantic_mask = Variable((semantic == 0).float() * -1000).cuda()  # [batch_size*5, seman_len]
        semantic_embed = self.embed(Variable(semantic).cuda())  # [batch_size*5, seman_len, embed_size]
        ...
        state_d = self.init_hidden_with_feature(features)  # [1, batch_size*5, hidden_size]

        last_word = Variable(torch.FloatTensor(batch_size).long().zero_()).cuda()
        outputs = []

        for i in range(self.seq_length):
            context = self.semantic_attention(semantic_embed, state_d, semantic_mask)
            log_probs, state_d = self.decode(out_e, last_word, state_d, context)
    ...
    def sample(self, features, semantic, sample_max, rl_training=False, pad=False):
        # encode the visual features
        ...
        for t in range(self.seq_length):
            last_word = Variable(last_word)
            context = self.semantic_attention(semantic_embed, state_d, semantic_mask)
            log_probs, state_d = self.decode(out_e, last_word, state_d, context, True)
            if t < 6:
    ...
    def predict(self, features, semantic, beam_size=5):
        assert beam_size <= self.vocab_size and beam_size > 0
        if beam_size == 1:  # if beam_size is 1, then do greedy decoding, otherwise use beam search
            return self.sample(features, semantic, sample_max=True, rl_training=False)
        ...
        for k in range(batch_size):
            ...
            semantic_embed_k = semantic_embed[k, :, :].unsqueeze(0).expand(beam_size, -1, -1).contiguous()
            semantic_mask_k = semantic_mask[k, :].unsqueeze(0).expand(beam_size, -1).contiguous()
            context = self.semantic_attention(semantic_embed_k, state_d_k, semantic_mask_k)
            last_word = Variable(torch.FloatTensor(beam_size).long().zero_().cuda())  # <BOS>
            log_probs, state_d_k = self.decode(out_e_k, last_word, state_d_k, context, True)
            log_probs[:, 1] = log_probs[:, 1] - 1000  # never produce <UNK> token
            neg_log_probs = -log_probs
